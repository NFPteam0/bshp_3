...
class Processor:
    ...
    def fit(self, data: pd.DataFrame):
        ...
        self.db_connector.update_status(
            "Model_info", "fitting_start_date", datetime.now(timezone.utc)
        )
        ...
        for target in self.targets:
            if target == "article_cash_flow":
                ...
                for name in names_art:
                    df_temp = df_encode[
                        df_encode["article_cash_flow"] == names_art[name]
                    ]
        ...
        self.db_connector.update_status(
            "Model_info", "fitting_end_date", datetime.now(timezone.utc)
        )
    ...
    def predict(self, data: pd.DataFrame):
        ...
        for row in range(len(data)):
            ...
            preds = MODEL_ART[mod_name].predict(
                data_to_predict[row : row + 1].drop(
                    columns=[
                        "document",
                        "article_cash_flow",
                        "details_cash_flow",
        ...
        decode_preds = decode_objects(
            self.db_connector, "article_cash_flow", data_to_predict["article_cash_flow"]
        )
        data_result["article_cash_flow"] = decode_preds

        for row in range(len(data)):
            mod_name = "model_details" + str(
                data_to_predict["article_cash_flow"].iloc[row]
            )
            preds = MODEL_DET[mod_name].predict(
                data_to_predict[row : row + 1].drop(
                    columns=["document", "details_cash_flow", "year"], axis=1
                )
            )
            data_to_predict["details_cash_flow"][row] = preds[0]
            logger.info("details_cash_flow--------DONE")
        decode_preds = decode_objects(
            self.db_connector, "details_cash_flow", data_to_predict["details_cash_flow"]
        )
        data_result["details_cash_flow"] = decode_preds

        year_time = time.time()
        preds = MODEL_YEAR.predict(
            data_to_predict.drop(columns=["document", "year"], axis=1)
        )
