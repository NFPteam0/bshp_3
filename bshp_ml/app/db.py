...
class DBProcessor:
    ...
    async def connect(self, url, timeout=25_000) -> bool:
        self.url = url
        self.timeout = timeout

        attempts = 3
        connected = False
        connection_timeout_error = None
        for att in range(1, attempts + 1):
            ...
            try:
                ...
                connection_timeout_error = e

        if connected:
            logger.info("Successfully connected to the database")
        else:
            logger.info(
                "Error database connection. {}".format(str(connection_timeout_error))
            )
    ...
    async def find(
        self, collection_name: str, db_filter=None, batch_size=0
    ) -> list[dict]:
        collection = self._get_collection(collection_name)
